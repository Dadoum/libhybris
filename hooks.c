...
static void* _hybris_hook_dladdr(void *addr, Dl_info *info)
{
    ...
    return (void *)android_dladdr(addr, info);
}
...
#ifdef WANT_ARM_TRACING
	android_linker_init(sdk_version, __hybris_get_hooked_symbol, enable_linker_gdb_support, create_wrapper);
#else
